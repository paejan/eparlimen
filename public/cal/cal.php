		...
		<table width=100% border="0" style="height:50px">
		  <tr> 
			...
			<td valign="middle" width="100%" align="left">
                ...
                <div class="col-md-3">
                <a href="peg_bertugas/pegawai_add.php?sel_mth=<?=$mth;?>&sel_year=<?=$yr;?>" data-toggle="modal" data-target="#myModal"  
			...
			if($intAllDays >= $intStartDayOfMonth && $intDays <= $intDayInMonth){
			   ...
			   if($j==0 || $j==5 || $j==6){ 
					...
					while(!$result->EOF){
						...
						if(!empty($result->fields['pegawai1'])){ print "<br>- " . dlookup("kakitangan","nama_kakitangan","id_kakitangan=".tosql($result->fields['pegawai1'],"Number")); }
						if(!empty($result->fields['pegawai2'])){ print "<br>- " . dlookup("kakitangan","nama_kakitangan","id_kakitangan=".tosql($result->fields['pegawai2'],"Number")); }
						if(!empty($result->fields['pegawai3'])){ print "<br>- " . dlookup("kakitangan","nama_kakitangan","id_kakitangan=".tosql($result->fields['pegawai3'],"Number")); }
						print '</font>';
						$result->movenext();
					}
					echo "</td></tr></table>";
					...
					if($intDays == $curr_dt  && $intMonth == $curr_mt){
						...
						while(!$result->EOF){
							...
							$form = "index.php?data=".base64_encode($user.';peg_bertugas/add_pegawai_form.php;'.$module.';'.$menu.';;;'.$jad_id)."&month=".$month."&yr=".$yr;
							//$row = mysql_fetch_array($result);
							//if(mysql_num_rows($result)<>0){
							print '&nbsp;&nbsp;<br><a href="peg_bertugas/add_pegawai.php?id='.$jad_id.'" data-toggle="modal" data-target="#myModal"  
                            	title="Selenggaran maklumat pegawai bertugas" class="fa" data-backdrop="static" style="font-family:verdana">
								<img src="images/edit.png" style="cursor:pointer" title="Sila klik untuk mengemaskini nama pegawai."> - <u>'.$dewan.'</u></a>'; 
							print '&nbsp;&nbsp;<img src="images/edit.png" style="cursor:pointer" onclick="do_page(\''.$form.'\')" title="Kemaskini Maklumat Pegawai">';
							//}
							print '<font size=1 face=Verdana, Arial, Helvetica, sans-serif color="#000000">';
