...
class PengesahanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //Soalan
        $soalan = SoalanParlimen::where('status',2)->where('is_semak',9)->where('is_sah',0)->where('is_deleted',0);
        ...
        return view('jawapan/pengesahan/index',compact('sidang','kategori','soalan'));
    }
    ...
    {
        // dd($id);
        $usermenu = TableMenuUser::where('menu_id',30)->where('id_kakitangan',Auth::user()->id_kakitangan)->first();
        ...
        $soal_semak = SoalanSemakan::where('semakan_jenis','SAH')->where('semakan_status',0)->where('soalan_id',$id)->first();
        // dd($soal_semak);
        if(empty($soal_semak->tkh_buka)){
            $sql = SoalanSemakan::where('semakan_status',0)->where('soalan_id',$id)->update(['tkh_buka'=>now()]);
        }
        // dd($ahli);
        return view('jawapan/pengesahan/form',compact('usermenu','title','soalan','ahli','attach','semak','komen','soal_semak'));
    }
    ...
    public function store(Request $request, $type)
    {
        ...
        if($type == 'UPDATE'){
            ...
            $data = array(
                'kenyataan'=>$request->kenyataan_text,
                'tkh_kemaskini'=>now(),
            ...
            if($soalan){
                ...
                $soalan_p = SoalanParlimen::where('soalan_id',$request->soalan_id)->update(['status'=>1,'is_semak'=>0,'is_sah'=>0]);
            } else {
                return response()->json('ERR');
            }
        } else if($type == 'SAH'){
            ...
            if($soalan){
                ...
                $soalan_p = SoalanParlimen::where('soalan_id',$request->soalan_id)->update(['status'=>2,'is_semak'=>9,'is_sah'=>9]);
            } else {
                return response()->json('ERR');
            }
        }
