...
function do_simpan(val){
  var reg = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,4}$/i;
	...
	var jawapan_semakan = CKEDITOR.instances['jawapan_semakan'].getData(); 
	document.jakim.jawapan_semakan_text.value=jawapan_semakan;
	
  $.ajax({
		url:'/soalan/jawapan_semakan/store/'+val, //&datas='+datas,
		type:'POST',
		//dataType: 'json',
		beforeSend: function () {
				...
				swal({
				  ...
				  showConfirmButton: true,
					...
					if(val=='UPDATE'){
						...
						window.location.href = "/soalan/jawapan_semakan";
					}
				});
			}
		}
            ...
            <div class="active tab-pane" id="profile">
              <div class="box box-info">
                ...
                <div class="form-group">
                  <label class="control-label col-md-2 col-xs-12" for="tkh_soalan"><b>Komen Terdahulu :</b></label>
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-10 col-sm-12 col-xs-12">@if(!empty($soal_semak->tkh_hantar)) {{ date('d/m/Y H:i:s', strtotime($soal_semak->tkh_hantar)) }} @endif</div>
                </div>

                <div class="form-group">
                  <label class="control-label col-md-2 col-xs-12" for="tkh_soalan"><b>Tarikh dibuka :</b></label>
                  <div class="col-md-10 col-sm-12 col-xs-12">@if(!empty($soal_semak->tkh_hantar)) {{ date('d/m/Y H:i:s', strtotime($soal_semak->tkh_buka)) }} @else {{ date('d/m/Y H:i:s') }} @endif</div>
                </div>

                <div class="form-group">
                  <label class="control-label col-md-2 col-xs-12" for="tkh_soalan"><b>Ulasan Pegawai Penyemak :</b>
                    <br /><br />
                  ...
                  <div class="col-md-10 col-sm-12 col-xs-12">
                    <textarea name="jawapan_semakan" cols="50" rows="10" id="jawapan_semakan" style="width:100%">@if(!empty($soal_semak->kenyataan)) {!! $soal_semak->kenyataan !!} @endif</textarea>
                  </div>
                </div>
                <div class="form-group">
                  <label class="control-label col-md-2 col-xs-12" for="tkh_soalan"><b>Tarikh dikemaskini :</b></label>
                  <div class="col-md-10 col-sm-12 col-xs-12">@if(!empty($soal_semak->tkh_kemaskini)) {{ date('d/m/Y H:i:s', strtotime($soal_semak->tkh_kemaskini)) }} @endif</div>
                </div>
							  <hr />
                ...
                <div class="form-group">
                  <div class="col-md-12 col-sm-12 col-xs-12" align="center">
                    ...
                    <textarea name="jawapan_semakan_text"  style="display:none;"></textarea>
                  </div>
                </div>
              </div>
					  </div>
...
<script>
	CKEDITOR.replace( 'jawapan_semakan' );
</script>
@endsection