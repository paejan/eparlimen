  ...
  var reg = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,4}$/i;
    ...
    if(tkh_soalan.trim() == '' ){
      ...
      $('#mailContents').focus(); return false;
		...
		$.ajax({
			url:'/soalan/daftar_form/store', //&datas='+datas,
			type:'POST',
			//dataType: 'json',
			beforeSend: function () {
...
function do_hapus(id_ap){
	swal({
		...
		$.ajax({
			url:'/soalan/daftar_form/delete/'+id_ap, //&datas='+datas,
			type:'POST',
			//dataType: 'json',
			/*beforeSend: function () {
				$('#simpan').attr("disabled","disabled");
...
function SelectData(){
    ...
    $.get('/soalan/daftar_form/'+bahagian,function(data){
        ...
        <div class="active tab-pane" id="profile">
          <div class="box box-info">
            <div class="box-header with-border">
              ...
              <div class="box-header with-border">
                <h3 class="box-title"><b> @if(empty($id))Tambah @else Kemaskini @endif Maklumat Daftar Soalan Dewan @if ($j_dewan == '1') Rakyat @else Negara @endif</b></h3><br />
              </div>
              <!-- /.box-header -->
            ...
            <div class="box-body">
              ...
              <div class="form-group">
                ...
                <div class="col-md-3 col-sm-12 col-xs-12">
                  <select name="j_tanya" id="j_tanya" class="form-control"  onchange="do_tanya(this.value)">
                    ...
                    <option value="{{ $tanya->j_tanya }}" @if($j_tanya == $tanya->j_tanya) selected  @endif>{{ $tanya->pertanyaan }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="col-md-3 col-sm-12 col-xs-12" id="tanya_detail">
                  <select name="j_tanya_det" id="j_tanya_det" class="form-control" disabled>
                    <option value=""></option>
                    <option value="MQT" @if($j_tanya_det == 'MQT') selected @endif>Pertanyaan MQT</option>
                    <option value="LISAN" @if($j_tanya_det == 'LISAN') selected @endif>Pertanyaan LISAN</option>
                    <option value="KAMAR KHAS" @if($j_tanya_det == 'KAMAR KHAS') selected @endif>Pertanyaan KAMAR KHAS</option>
                    <option value="USUL" @if($j_tanya_det == 'USUL') selected @endif>Pertanyaan USUL</option>
                  </select>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-10 col-sm-12 col-xs-12">
                  <select name="id_sidang" id="id_sidang" class="form-control">
              ...
              <div class="form-group">
                <label class="control-label col-md-2 col-xs-12" for="s_oleh">
                  @if (Request::is('soalan/daftar_form_dr'))
                  <b>Kawasan Parlimen / YB :</b>
                  @else
                  <b>Yang Berhormat :</b>
                  @endif
                </label>
                <div class="col-md-10 col-sm-12 col-xs-12">
                  ...
                  <select name="nama_ap" id="nama_ap" onChange="do_sel()" class="form-control input-lg" data-live-search="true" style="height:30px">
                    ...
                    @foreach($ahli as $ap)
                    <option value="{{ $ap->id_ap }}" @if($s_oleh == $ap->id_ap) selected @endif>{{ $ap->kod_kaw_ap }} : {{ $ap->p_nama }} - {{ $ap->nama_ap }} ({{ $ap->parti }})</option>
                    @endforeach
                    @else
                    <option value=""> -- Sila Pilih Nama Yang Berhormat -- </option>
                    @foreach($ahli as $ap)
                    <option value="{{ $ap->id_ap }}" @if($s_oleh == $ap->id_ap) selected @endif>{{ $ap->nama_ap }} - {{ $ap->kawasan_ap }}</option>
                    @endforeach
                    @endif
                  </select>
              ...
              <div class="form-group">
                ...
                <div class="col-md-3 col-sm-12 col-xs-12">
                {{ $tkh_masuk}}
                  <input name="tkh_masuk" id="tkh_jwb_parlimen" type="hidden" class="form-control" value="{{ $tkh_masuk}}" />
                </div>
              </div>
