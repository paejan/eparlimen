...
function do_page()
{
    ...
    window.location = '/peg_bertugas/senarai?sel_mth='+sel_mth+'&sel_year='+sel_year;
}
        ...
        <table width=100% border="0" style="height:50px">
          <tr> 
            <td valign="middle" nowrap width="20%"align="left">&nbsp;
              <a href="senarai?prevMonth=<?=$prevMonth;?>&curYear=<?=$prevYear;?>&p=PREV">
            ...
            <td valign="middle" width="100%" align="left">
              <div align="center">
                <font size="1" face="Verdana, Arial, Helvetica, sans-serif" color="#000000">
                  ...
                  <div class="col-md-3">
                    <input type="button" value="Cetak Jadual" style="cursor:pointer" class="form-control btn btn-primary" onClick="do_cetak('peg_bertugas/cetak_peg_view.php?pro=BLN')" />
                  </div>
                </font>
              </div>
            </td>
            <td valign="middle" nowrap width="20%" align="right">
              <a href="senarai?nextMonth=<?=$nextMonth;?>&curYear=<?=$nextYear;?>&p=NEXT">
        ...
        <td height=60 valign=top class=Days>
          @if ($intAllDays >= $intStartDayOfMonth && $intDays <= $intDayInMonth)
            ...
            @if ($j==0 || $j==5 || $j==6)
                ...
                @foreach ($result_t->where('jad_tkh',$dateDisp) as $rst)
                  <font size=1 face=Verdana, Arial, Helvetica, sans-serif color="#000000">
                  <input type="hidden" value="{{ $rst->id_sidang }}" name="id_sidang" id="id_sidang"/>
            ...
            @else
                @if ($intDays == $curr_dt  && $intMonth == $curr_mt)
                  <table width=100% bgcolor=#B9CAE6 border=0><tr><td title=Today style="cursor:pointer;padding-left:5px;padding-right:5px;" align=left>
                    ...
                    @foreach ($result_p->where('jad_tkh',$dateDisp) as $rsp)
                      &nbsp;&nbsp;<br><a href="/peg_bertugas/senarai/view/{{ $rsp->jad_id }}" data-toggle="modal" data-target="#myModal" title="Paparan maklumat pegawai bertugas" class="fa" data-backdrop="static" style="font-family:verdana"><u><b>{{ $rsp->dewan }}</b></u></a>
                      <font size=1 face=Verdana, Arial, Helvetica, sans-serif color="#000000">
                      <input type="hidden" value="{{ $rsp->id_sidang }}" name="id_sidang" id="id_sidang"/>
                      @if(!empty($rsp->pegawai1)) <br>P1- {{ $rsp->p1->nama_kakitangan }} @endif
                      @if(!empty($rsp->pegawai2)) <br>P2- {{ $rsp->p2->nama_kakitangan }} @endif
                      @if(!empty($rsp->pegawai3)) <br>P3- {{ $rsp->p3->nama_kakitangan }} @endif
                      <br>
                      @if(!empty($rsp->urusetia)) <br>UR - {{ $rsp->us->nama_kakitangan }} @endif
                      @if(!empty($rsp->pemandu)) <br>PE - {{ $rsp->pm->nama_kakitangan }} @endif
                      </font><br>
                    @endforeach
                  </td></tr></table>
                @else
